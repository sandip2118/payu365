...
const SecuritySettings:React.FC<SecuritySettingsProps>=({tag,icon,iconPath,title,description,extra,extraText,switch:isSwitchOn,switchIcon,switchText,buttonText ,isBorder,route,tabTitle})=> {
  const router = useRouter();
  const navigateToPage = (route, tabTitle) => {
    if (route) {
  ...
  return (
    <div className={`grid lg:flex gap-[10px] md:gap-[0px] py-0 pb-5  md:py-2.5 mt-[16px] md:mt-5 ${isBorder ? 'border-b-[1px] border-[#44444F]': ''}`}>
    ...
    <div className='flex justify-end'>
          <button className='font12RB text-[#B1B5C3] bg-[#48546D] rounded-[5px] w-[87px] h-[28px]'onClick={()=>navigateToPage(route,tabTitle)} >{buttonText}</button>
        </div>
    </div>
    </div>
  )

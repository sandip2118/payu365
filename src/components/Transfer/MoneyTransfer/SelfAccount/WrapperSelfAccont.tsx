...
import QuickBank from './QuickBank';
...
function WrapperSelfAccont({ setOptions }: any) {
  const router = useRouter()
  const matches = useMediaQuery('(min-width:600px)');

  const [activeStep, _activeStep] = useState<any>(0);
  const [selectQuickBank, setSelectQuickBank] = useState('')
  const steps = [
    { id: 1, name: 'Select Self A/C' },
  ...
  return (
    <>
      ...
      {activeStep === 0 &&
        <SelfAccount activeStep={activeStep} _activeStep={_activeStep} />
      }
      {activeStep === 1 &&
        <VerifyAmount activeStep={activeStep} _activeStep={_activeStep} />
      }
          ...
          <SuccessBox activeStep={activeStep} _activeStep={_activeStep} setOpenReceipt={setOpenReceipt}
            texts={
              <>
                ...
                <p className="font18SB text-center mt-0">
                  to your Self Account
                </p>
                ...
                <p className="font12RB text-GRAY_101 !leading-[18px] text-center mt-0">
                  ICICI Bank-1704  from your wallet/Account
                </p>
              </>
            }
          />
...
export default WrapperSelfAccont
